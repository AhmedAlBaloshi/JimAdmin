...
@Component({
  selector: 'app-minor-categories',
  templateUrl: './minor-categories.component.html',
  styleUrls: ['./minor-categories.component.scss']
})
export class MinorCategoriesComponent implements OnInit {
  ...
  getMinorCategories() {
    ...
    this.dummy = Array(10);
    this.api.get('minorcategories').then((res: any) => {
      this.dummy = [];
  ...
  deleteIt(item) {
    Swal.fire({
      ...
      if (status && status.value) {
        ...
        this.spinner.show();
        this.api.post('minorcategories/deleteList', param).then((res: any) => {
          this.spinner.hide();
