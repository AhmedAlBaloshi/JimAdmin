...
export class ManageStoresComponent implements OnInit {
  ...
  paymentMethod = [{"type":"cash","title_en":"Cash", "title_ar":"الدفع عند الاستلام"},{"type":"online","title_en":"Credit Card", "title_ar":"بطاقة الإئتمان"}];
  ...
  initMap() {
    ...
    this.mapbox.on("click", (ev) => {
      ...
      fetch("https://api.mapbox.com/geocoding/v5/mapbox.places/" +
        ev.lngLat.lng + "," + ev.lngLat.lat + ".json?country=om&limit=1&types=place&language=ar%2Cen&access_token=" + environment.mapbox.accessToken)
        .then((response) => {
          if (response.ok) {
  ...
  updateVenue() {
    if (this.name_en === '' || this.name_ar === '' || this.address_en === '' || this.address_ar === '' || this.descriptions_en === '' || this.descriptions_ar === '' || this.openTime === '' || this.closeTime === ''
      || !this.openTime || !this.closeTime || this.deliveryTime === '' || this.shippingPrice === '') {
  ...
  create() {
    if (this.email === '' || this.fname === '' || this.lname === '' || this.phone === '' || this.password === ''
      || this.name_en === '' || this.name_ar === '' || this.address_en === '' || this.address_ar === '' || this.descriptions_en === '' || this.descriptions_ar === ''
      || this.city === '' || !this.city || this.openTime === '' || this.closeTime === '' ||
      !this.openTime || !this.closeTime || this.minOrderPrice === '' || this.deliveryTime === '' || this.shippingPrice === '') {
      this.error(this.api.translate('All Fields are required'));
      return false;
    }
