...
@Component({
  selector: 'app-manage-major-categories',
  templateUrl: './manage-major-categories.component.html',
  styleUrls: ['./manage-major-categories.component.scss']
})
export class ManageMajorCategoriesComponent implements OnInit {
  ...
  getMajorCategories() {
    ...
    this.spinner.show();
    this.api.post('majorcategories/getById', param).then((res: any) => {
      this.spinner.hide();
  ...
  update() {
    ...
    this.api.post('majorcategories/editMajorCategory', param).then((res: any) => {
      this.spinner.hide();
      if (res && res.data && res.status === 200) {
        this.navCtrl.back();
      } else {
